...
namespace scy {
    namespace sockio {
        ...
        Packet::Packet(Frame frame, Type type, int id, const std::string &nsp,
                ...
                : _frame(frame), _type(type), _id(id), _nsp(nsp), _event(event), _message(message), _ack(ack), _size(0) {
        }
        ...
        Packet::Packet(const Packet &r)
                : _frame(r._frame), _type(r._type), _id(r._id), _nsp(r._nsp), _event(r._event), _message(r._message),
                  _ack(r._ack), _size(r._size) {
        }
        ...
        ssize_t Packet::read(const ConstBuffer &buf) {
            ...
            if (reader.available()) {
                ...
                json::value json;
                try {
                json = json::value::parse(temp.begin(), temp.end());
                } catch (const std::exception &e) {
                  std::cout << e.what() << " " << temp << std::endl;
                }
                if (json.is_array()) {
                    if (json.size() < 2) {
                        _event = "message";
