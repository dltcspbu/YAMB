...
//
    ...
    Client::Client(const net::Socket::Ptr &socket, const Client::Options &options)
        : _pingTimer(socket->loop()), _pingTimeoutTimer(socket->loop()), _reconnectTimer(socket->loop()),
          _options(options), _ws(socket), _wasOnline(false) {
...
//
    ...
    void Client::onSocketRecv(net::Socket &socket, const MutableBuffer &buffer, const net::Address &peerAddress) {
      ...
      if (len == buffer.size()) LWarn("Failed to parse incoming Socket.IO packet.")
