...
//
    ...
    Client::Client(const net::Socket::Ptr &socket, const Client::Options &options)
        : sockio::Client(socket), _options(options), _announceStatus(0) {
      LTrace("Create")
    }
    ...
    int Client::sendPresence(const Address &to, const std::string &room, bool probe) {
      LTrace("Sending presence")
    ...
    void Client::onPresenceData(const json::value &msg, bool whiny) {
      auto data = msg["data"];
      LDebug("Presence", msg.dump())
      ...
      if (data.is_object() &&
        ...
        if (msg.find("room") != msg.end()) {
          id += msg["room"].get<std::string>();
        }
        ...
        if (online) {
          ...
          if (peer) {
            ...
            PeerDisconnected.emit(*peer, msg);
            _roster.free(id);
          } else {
