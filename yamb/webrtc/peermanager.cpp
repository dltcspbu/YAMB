...
namespace scy {
  namespace wrtc {
    ...
    void PeerManager::recvSDP(const std::string &token, const json::value &message,
                              const std::vector<scy::json::value> &iceCandidates) {
      auto conn = PeerManager::get(token, false);
      if (!conn) {
        assert(0 && "peer mismath");
        return;
    ...
    void PeerManager::recvCandidate(const std::string &token, const json::value &message) {
      auto conn = PeerManager::get(token, false);
      if (message.find("sdptype") != message.end()) {
        const auto &mode = Peer::Mode(message["sdptype"]);
        if (mode == conn->mode())
          return;
      }
      ...
      const auto pp = message.dump();
      const auto p = message.find("candidate") != message.end();
//      const auto &candidate =
//          (message.find("candidate") != message.end() ? message["candidate"].get<scy::json::value>()
//                                                      : message).get<scy::json::value>();
