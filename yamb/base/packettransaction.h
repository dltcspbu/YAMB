...
/// after a successful response or a timeout.
  ...
  public:
    PacketTransaction(long timeout = 10000, int retries = 0,
        ...
        : _timer(timeout, loop), _loop(loop), _retries(retries), _attempts(0), _destroyed(false) {
    }

    PacketTransaction(const PacketT &request, long timeout = 10000,
                      int retries = 0, uv::Loop *loop = uv::defaultLoop())
        : _request(request), _loop(loop), _timer(timeout, loop), _retries(retries), _attempts(0), _destroyed(false) {
    }
    ...
    virtual void dispose() {
      ...
      if (!_destroyed) {
        ...
        deleteLater<PacketTransaction>(this, _loop);
      }
    }
