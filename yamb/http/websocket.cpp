...
//
      ...
      WebSocketAdapter::WebSocketAdapter(const net::Socket::Ptr &socket,
          ...
          , socket(socket), framer(mode), _request(request), _response(response) {
        LTrace("Create")
...
//        auto size_b = buffer.size();
//        void *copyd = new char[size_b];
//        memcpy(copyd, buffer.data(), size_b);
//        MutableBuffer bufferlocal = MutableBuffer(copyd, size_b);
//        std::vector<MutableBuffer> tmpvec;
        if (framer.handshakeComplete()) {
          ...
          while (offset < total) {
            ...
            try {
              ...
              if (offset < total) LTrace("Splitting joined packet at ", offset, " of ", total)
...
//            std::cout << payloadLength << std::endl;
            //auto b = mutableBuffer(payload, (size_t) payloadLength);
            //tmpvec.push_back(b);
            net::SocketEmitter::onSocketRecv(*socket.get(), mutableBuffer(payload, (size_t) payloadLength),
                                             peerAddress);
          }
          assert(offset == total);
        } else {
...
//
      ...
      WebSocketFramer::WebSocketFramer(ws::Mode mode)
          : _mode(mode), _frameFlags(0), _headerState(0), _maskPayload(mode == ws::ClientSide) {
      }
      ...
      bool WebSocketFramer::checkClientHandshakeResponse(http::Response &response) {
        ...
        switch (response.getStatus()) {
          ...
          case http::StatusCode::Unauthorized: {
            ...
            // }
          default:
            throw std::runtime_error("WebSocket error: Cannot upgrade to WebSocket connection: " +
      ...
      void WebSocketFramer::acceptServerRequest(http::Request &request, http::Response &response) {
        ...
        if ((util::icompare(request.get("Connection", ""), "upgrade") == 0 ||
          ...
          if (version.empty())
            throw std::runtime_error(
                "WebSocket error: Missing Sec-WebSocket-Version in handshake request"); //, ws::ErrorHandshakeNoVersion
          if (version != ws::ProtocolVersion)
            throw std::runtime_error("WebSocket error: Unsupported WebSocket version requested: " +
                                     version); //, ws::ErrorHandshakeUnsupportedVersion
          std::string key = util::trim(request.get("Sec-WebSocket-Key", ""));
          if (key.empty())
            throw std::runtime_error(
                "WebSocket error: Missing Sec-WebSocket-Key in handshake request"); //, ws::ErrorHandshakeNoKey
      ...
      uint64_t WebSocketFramer::readFrame(BitReader &frame, char *&payload) {
        ...
        if ((lengthByte & 0x7f) == 127) {
          ...
          if (l > limit)
            throw std::runtime_error(
                util::format("WebSocket error: Insufficient buffer for payload size %" PRIu64,
